...
import { useTranslations, defineMessage } from '@/lib/i18n'
import { useAuth } from '@/lib/auth-context'

const baseLinks = [
	{
		label: defineMessage('Home'),
		href: '/',
                icon: (
			<svg
				viewBox='0 0 15 15'
...
const adminLink = {
	label: defineMessage('Admin Panel'),
        href: '/admin',
        icon: (
		<svg
			viewBox='0 0 15 15'
			...
			<nav className='fixed bottom-0 w-full border-t bg-zinc-100 pb-safe dark:border-zinc-800 dark:bg-zinc-900'>
				...
				{links.map(({ href, label, icon }) => (
					<Link
						...
						>
							{icon}
						<span className='text-xs text-zinc-600 dark:text-zinc-400'>{t(label)}</span>
					</Link>
				))}
				</div>
			</nav>
